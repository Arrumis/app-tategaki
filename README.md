...
- 小説家になろう、カクヨム、ノクターンノベルズの作品をローカル保存
- ノクターンは18歳以上向け。`https://novel18.syosetu.com/n1234ab/` 形式の作品URL（各話URLも可）を登録する。取得時に年齢確認が表示された場合は公式の入場ボタンで進み、同じブラウザセッション内でCookieを保持する。
- ノクターンの更新確認には[なろうR18小説API](https://dev.syosetu.com/xman/api/)を使用し、目次・本文は作品ページから取得する。APIが利用できない場合はページ取得に切り替える。
- ブラウザで縦書き読書
- お気に入りリスト
- しおりと閲覧履歴
- 定期巡回
