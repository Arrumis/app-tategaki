...
### 1-1. バックエンド アプリ連携処理 拡張 (`src/routes/api.js`, `src/crawler/manager.js`)
*   **クローラー起動アプリ連携処理の修正**:
...
### 1-2. フロントエンド 画面 実装 (`public/index.html`, `public/js/home.js`)
...
*   **小説追加モーダル**:
    ...
    *   アクション: アプリ連携処理をコールし、「バックグラウンドで処理を開始しました」とトースト表示。
...
*   **目次オーバーレイ**:
    ...
    *   アプリ連携処理 (`GET /novels/:site/:id/info` または専用アプリ連携処理) からエピソード一覧を取得し、リスト表示。
    *   クリックで該当エピソードへ遷移。
...
## 開発順序の提案
1.  **作業 1 (新規登録)**: アプリとして「新しい本を追加できない」のは致命的なため最優先。
2.  **作業 2 (リーダー強化)**: 「読む」体験の質を向上。
3.  **作業 3 (自動化)**: 運用機能。
4.  **作業 4 (Docker)**: 配布・デプロイ準備。
